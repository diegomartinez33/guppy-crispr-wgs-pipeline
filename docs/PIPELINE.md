...
10. [RT-qPCR Primer Verification](#10-rt-qpcr-primer-verification)
...
**Note:** this section only covers the *automated design* of new primers around already-known
on-/off-target sites. Verifying *existing* RT-qPCR primers (designed elsewhere, e.g. for a
housekeeping gene) is a different task with different failure modes — see
[§10](#10-rt-qpcr-primer-verification).

---

## 10. RT-qPCR Primer Verification

**Purpose:** check whether an existing RT-qPCR primer pair (not designed by this project) still
binds correctly in the Colombian population, in the correct genome versions, and — critically —
within real mRNA/exonic sequence rather than genomic DNA that never survives splicing. This is a
fundamentally different check from [§9](#9-pcr-primer-design)'s on-/off-target primer design: RT-qPCR
amplifies mRNA/cDNA, so a primer's binding site must fall within a real, spliced exon (or exactly
span a real exon-exon junction) — unlike CRISPR on-/off-target primers, which correctly and
intentionally target raw genomic DNA (Cas9 cuts genomic DNA, not mRNA).

```mermaid
flowchart TD
    SEQ["primer sequence(s)\n(CSV: pair_name, forward_seq, reverse_seq)"] --> LOC["1. LOCATE\nblastn-short vs whole genome\n(ALL hits kept, not just the top one)"]
    REF["reference genome(s)\n(v1 and/or v2)"] --> LOC
    LOC --> ANN["2. ANNOTATE\nGFF spatial overlap: any gene\nregardless of symbol (catches LOC######## genes)\nsmallest-span gene wins if genes are nested"]
    ANN --> CAND{"candidate\ngene(s) found?"}
    CAND -- no --> NOGENE["GENOMIC_MATCH_NO_GENE\n(matches DNA, but not inside any gene)"]
    CAND -- yes, several --> REALIGN["3. REALIGN each candidate\nspliced mRNA (exons concatenated per GFF)\n+ EMBOSS water (indel-aware, not Hamming distance)\nkeep the LONGEST + highest-identity match"]
    REALIGN --> XVER["4. CROSS-CHECK\nrepeat 1-3 per --ref-versions\n(v1 may lack/fragment a gene v2 resolves, or vice versa)"]
    XVER --> POP["5. POPULATION CHECK\nexact-match, single-exon v1 hits only:\nliftover via .chain + direct sequence compare\nagainst the Colombian pseudogenome"]
    POP --> CSV["rtqpcr_primer_verification.csv"]
```

| Script | Purpose |
|---|---|
| `codes/analysis/verify_rtqpcr_primers.py` | Full checklist: `--primers-csv`, `--ref-versions v1,v2`, `--population-check` |
| `codes/analysis/run_rtqpcr_primer_verification.sh` | SLURM wrapper — `PRIMERS_CSV`/`REF_VERSIONS`/`POPULATION_CHECK` |

Reused verbatim (no reimplementation): `REF_FASTA_V1/V2`, `REF_GFF_V1/V2`, `faidx_seq()`,
`revcomp()` from `ko_guide_scan.py`; `REF_BY_VERSION`, `PSEUDOGENOME_BY_VERSION`,
`CHAIN_BY_VERSION`, `CROSSMAP_BIN`, `liftover_region()` from `design_offtarget_primers.py`.
...
python3 codes/analysis/verify_rtqpcr_primers.py \
  ...
  --population-check
```
`my_rtqpcr_primers.csv` columns: `pair_name,forward_seq,reverse_seq`.

**Why each step exists (all found the hard way, by hand, before being formalized here — see
`CLAUDE.md`, "RT-qPCR Primer Verification"):**
- **BLAST, not a fixed-length exact/Hamming scan, for locating the site.** A primer that spans a
  real exon-exon junction has no full-length contiguous genomic match at all — BLAST still finds
  the largest exonic sub-fragment; a plain substring search just returns nothing.
- **ALL BLAST hits are kept, not only the best one, until AFTER gene annotation.** A junction-
  spanning primer's true (but short) exonic fragment can score below dozens of coincidental,
  non-genic short matches elsewhere in the genome — capping the hit list too early can silently
  discard the correct site (verified against `rpl_13a_R`: capping at the top 25 raw hits dropped
  the true `rpl13a` hit in favor of an unrelated gene's coincidental 16bp match).
- **Gene lookup is by GFF spatial overlap, not gene-symbol regex.** Guppy's fast-skeletal-muscle
  myosin heavy chain genes are annotated as generic `LOC########` IDs with no short symbol — a
  `myh*`/`myo*`/`myl*`-style search misses them completely (this is exactly how
  `miosina_guppy_F/R` was first, wrongly, concluded to have "no match anywhere" — see
  `CLAUDE.md`). Spatial overlap has no such blind spot.
- **The smallest-span overlapping gene wins when genes are nested.** Large genes with big introns
  can fully enclose a smaller, unrelated gene (confirmed for real: bdnf's own 14.5kb span sits
  nested inside a neighboring gene's 201kb model) — taking "the first gene found" or the largest
  one picks the wrong target.
- **Every plausible candidate gene is realigned with EMBOSS `water` (indel-aware, both
  orientations), never a fixed-length Hamming scan.** A single 1bp indel makes a 19/20 (95%) real
  match look like "3 mismatches" under Hamming distance. Also: `water` almost always returns SOME
  local alignment even between unrelated sequences, so a plain `fwd or rev` short-circuit on the
  forward orientation practically never falls through to reverse-complement — both orientations
  must always be computed and compared explicitly, or the wrong (much weaker) orientation gets
  kept silently.
- **Candidate genes are ranked by `identity_n / max(identity_len, len(primer))`** — the fraction
  of the primer's own length correctly, non-redundantly explained by the alignment — not by
  identity% or identity count alone. Both simpler alternatives were tried and both broke on a real
  case: identity-count-first lets `water` stitch a long, heavily gapped "identity block" (e.g.
  19/39=48.7%, 19 gaps) that racks up more raw identical bases than a short, clean, real match
  (e.g. 18/20=90%, 0 gaps); identity%-first lets a short coincidental fragment be a "perfect"
  match over only part of the primer (e.g. 12/12=100%, covering 12 of 19bp) and out-rank a real
  match covering nearly the whole primer with one indel (e.g. 19/20=95%). The coverage-normalized
  ratio rejects both failure modes at once.
- **Every genome version requested is checked independently, not assumed transferable.** The same
  gene's transcript model can be short/fragmentary in one assembly and complete in another (seen
  for one myosin paralog: 3,632bp in v1 vs 6,033bp in v2, for the identical GeneID) — a primer
  can be reliable in v2 and unreliable in v1 for the same target.
- **The GFF is parsed into an in-memory index once per run, never re-scanned per lookup.** A
  whole-genome GFF here is 1.3-1.6M lines (350-470MB) — re-opening and linearly scanning it for
